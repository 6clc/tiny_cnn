...
namespace tinyDNN
{
  ...
  template <typename Dtype>
  MSE_Loss_LayerQL<Dtype>::MSE_Loss_LayerQL(LayerType type) : LayerQL<Dtype>(type)
  {
  ...
  template <typename Dtype>
  void MSE_Loss_LayerQL<Dtype>::calForward(int type ) const {}

  template <typename Dtype>
  void MSE_Loss_LayerQL<Dtype>::calBackward(int type )
  {
