...
namespace tinyDNN
{
  ...
  public:
    ...
    void pool_Average_Vector_Normal()
    {
      ...
      while (reTime > 0)
      {
        std::for_each(inLayer_01->forward_Matrix_Vector.begin(), inLayer_01->forward_Matrix_Vector.end(), [](std::shared_ptr<MatrixQL<double>> matrixTest)
                      {
                        std::cout << matrixTest->getMatrixQL() << std::endl;
                        std::cout << "�ػ�ǰ**********************" << std::endl;
                      });
        pool_Layer->calForward();
        std::for_each(inLayer_02->forward_Matrix_Vector.begin(), inLayer_02->forward_Matrix_Vector.end(), [](std::shared_ptr<MatrixQL<double>> matrixTest)
                      {
                        std::cout << matrixTest->getMatrixQL() << std::endl;
                        std::cout << "�ػ���**********************" << std::endl;
                      });
      ...
      while (reTime_02 > 0)
      {
        ...
        std::for_each(inLayer_01->backward_Matrix_Vector.begin(), inLayer_01->backward_Matrix_Vector.end(), [](std::shared_ptr<MatrixQL<double>> matrixTest)
                      { std::cout << (matrixTest->getMatrixQL()) << std::endl; });
        reTime_02--;
      }
    }
    ...
    void pool_Average_Vector()
    {
      ...
      std::for_each(inLayer_01->forward_Matrix_Vector.begin(), inLayer_01->forward_Matrix_Vector.end(), [](std::shared_ptr<MatrixQL<double>> matrixTest)
                    { std::cout << (matrixTest->getMatrixQL() * 9).cast<int>() << std::endl; });
